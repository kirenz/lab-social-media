...
df['Beitragsinteraktionen'] = df['Beitragsinteraktionen'].astype('Int64')
df['Geteilte Beiträge'] = df['Geteilte Beiträge'].astype('Int64')
df['Beitragskommentare'] = df['Beitragskommentare'].astype('Int64')
...
PLATTFORM = 'Facebook'
df_mean = df.groupby('Plattform')[KENNZAHLEN].mean().reset_index()
...
# als Prozent anzeigen
df_mean['CTR (Link-Klickrate)'] = df_mean['CTR (Link-Klickrate)'].map('{:.2%}'.format)
df_mean['Ergebnisrate'] = df_mean['Ergebnisrate'].map('{:.2%}'.format)    
...
```{python}
# | label: awareness
df[['Ausgegebener Betrag (EUR)', 'Reichweite', 'Impressionen', 'Kosten pro 1.000 erreichten Personen (EUR)', 'CPM (Kosten pro 1.000 Impressionen) (EUR)']].round(
...
- Durchschnitt aller Kampagnen auf der Plattform:
...
```{python}
# | label: awareness_mean
df_mean[['Ausgegebener Betrag (EUR)', 'Reichweite', 'Impressionen', 'Kosten pro 1.000 erreichten Personen (EUR)', 'CPM (Kosten pro 1.000 Impressionen) (EUR)']].round(
    0).style.pipe(make_pretty)
```
...
```{python}
# | label: engagement
df[['Beitragsinteraktionen', 'Geteilte Beiträge', 'Beitragskommentare']].round(
    0).style.pipe(make_pretty)
```
...
```{python}
# | label: engagement_mean
df_mean[['Beitragsinteraktionen', 'Geteilte Beiträge', 'Beitragskommentare']].round(
    0).style.pipe(make_pretty)
```
...
```{python}
# | label: conversion
df[['CTR (Link-Klickrate)', 'CPC (alle) (EUR)', 'Ergebnisrate']].round(
    2).style.pipe(make_pretty)
```
...
```{python}
# | label: conversion_mean
df_mean[['CTR (Link-Klickrate)', 'CPC (alle) (EUR)', 'Ergebnisrate']].round(
    2).style.pipe(make_pretty)
```
